...
// ---------------------------------------------------------------------------
// AIPOS-F56: 空闲带路追加一行可复制唤醒指令(Owner 面)
// ---------------------------------------------------------------------------

/**
 * AIPOS-F56: 构建 Owner 可复制的唤醒指令行。
 *
 * 场景:连接器检测到"已认领未投递"(held + IN_PROGRESS)时,除给模型的工位路标外,
 * 额外输出一行 Owner 可直接复制粘贴的唤醒指令。
 *
 * 内容要求(卡面验收):
 * ①含卡的绝对路径(Owner 无需再查)
 * ②含卡号与角色(便于确认贴对窗口)
 * ③不含顾问才知道的上下文(那些模型读卡即得)
 * ④自成一行、无前后缀装饰、可整行复制粘贴
 *
 * 该行走 voice() 单出口(F15),与 F44C⑦共用同一受众分级(禁各写一套)。
 * 节流遵循 F44C⑥(同卡同状态只出一次,由调用方 lastResumeVoice 控制)。
 */
export function buildOwnerWakeupLine(taskId: string, cardAbsPath: string, role: string): string {
  return `读卡 ${cardAbsPath} (${taskId}, ${role})`;
}
