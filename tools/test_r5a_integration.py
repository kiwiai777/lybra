...
AIPOS-R6C: 改用临时仓，断绝向 main 推送测试 commit 的源头。
"""
...
def _create_test_repo(tmp_dir: Path) -> Path:
    ...
    # 创建最小化的 .lybra 配置
    lybra_dir = repo_path / ".lybra"
    lybra_dir.mkdir()
    config = {
        "workspace_root": str(repo_path),
        "code_repo": str(repo_path),
        "worktree_root": str(repo_path / ".worktrees")
    }
    (lybra_dir / "workspace_config.json").write_text(json.dumps(config, indent=2))
    
    return repo_path


def test_claim_worktree_integration():
    """测试 claim 时创建 worktree 的集成（使用临时仓）。"""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # 无需清理测试文件（临时仓会自动删除）
            print(f"✓ Test file remains in temporary repo (will be auto-cleaned)")
...
def test_parallel_worktrees():
    """测试并行 worktree 场景（FND-19 主诉，使用临时仓）。"""
...
def test_no_main_pollution():
    """AIPOS-R6C 断言：测试运行后产品仓 main 无新 commit。"""
    print("\n" + "=" * 60)
    print("Testing no main pollution (R6C requirement)")
    print("=" * 60)
    ...
    print(f"HEAD before tests: {head_before}")
    ...
    print(f"HEAD after tests: {head_after}")
    
    assert head_before == head_after, f"Main branch was polluted! Before: {head_before}, After: {head_after}"
    print("✓ Verified: main branch unchanged after test run")
    print("✓✓✓ No main pollution test passed!")


if __name__ == "__main__":
    print("=" * 60)
    print("AIPOS-R5A Integration Tests (R6C: isolated to temp repos)")
    print("=" * 60)
    
    test_no_main_pollution()
