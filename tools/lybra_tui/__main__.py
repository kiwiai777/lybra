...
# The global agent-side credential is ~/.lybra/agent_credentials.json (remote agent use only).
# --connection-json overrides both.
CONNECTION_REL = ".lybra/connection.json"
RUNTIME_CONNECTION = Path("~/.lybra/agent_credentials.json")


def default_connection_json(workspace_root: str | None) -> str | None:
    """AIPOS-349: resolve the connection.json, workspace first.

    Prefers <workspace>/.lybra/connection.json (the canonical workspace location);
    falls back to ~/.lybra/agent_credentials.json (remote agent-side credential) only
    when no workspace root is given or the workspace file does not exist.
    Returns the path string if a file exists, else None. Reads no token (path only).
    """
