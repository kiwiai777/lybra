...
def validate_draft_metadata(
    ...
    *,
    ...
    if SCHEMA_AVAILABLE:
        ...
        # (from both rules AND _suspended_rules) so we don't warn about them as "unknown".
        card_policy_fields: set[str] = set()
        if CARD_POLICY_AVAILABLE:
            from tools.card_policy_loader import get_card_policy_all_declared_fields
            card_policy_fields = get_card_policy_all_declared_fields(governance_root=repo_root, repo_root=None)
