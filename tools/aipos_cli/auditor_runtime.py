...
def launch_auditor_runtime(
    ...
    kickoff = (
        ...
        f"\n\n## 裁决落库纪律 (AIPOS-FND-15 固化)"
        f"\n【红线】裁决**必须**通过 `lybra audit-verdict` 命令过 gate 落真 audit_verdict_record,"
        f"\n**永不手写** markdown 到 records/audit_verdicts/ (手写裁决会被 finalize 拒绝)。"
        f"\n命令形式: `lybra audit-verdict --reviewed-task-id <原卡ID> --actor <你> --agent-instance <你> "
        f"\n  --owner-policy-ref <policy> --verdict PASS/FAIL --findings-summary '<摘要>' "
        f"\n  --audit-claim-id <你的claim_id> --audit-session-id <你的session_id> "
        f"\n  --gate-url <gate> --connection-json <连接配置> --token-role auditor`。"
        f"\n该命令内部走 {verdict_verb_name} + confirm 两阶段,自动落库。"
        f"\n\n## 裁决提交失败规范动作"
        f"\n如果 `lybra audit-verdict` 返回错误:"
        f"\n1. 将完整错误 JSON 贴到报告末尾"
        f"\n2. 写 blocked 事件到 {events_abs_glob}"
        f"\n3. 禁止手写 records (重申: 手写裁决 = 违规,会被拒)"
        f"\n4. 如实退出 (非零 exit)"
    )
